...
- **dApp awareness.** Known dApp scripts are classified into a dedicated
  ...
  - [Liqwid](https://liqwid-labs.gitbook.io/liqwid-docs) — supply / withdraw
    liquidity across markets (qToken mint / burn). Market list from the
...
- `static/` - the whole frontend (HTML/CSS/JS + logos/images); no frameworks,
  no build step; embedded via `include_str!` / `include_bytes!`. DEX brand
  UI lives under `static/dex/`; optional dApp UI under `static/dapp/`.
